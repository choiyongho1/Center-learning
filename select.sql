SELECT * FROM MEMBER WHERE COLUMN LIKE '%INCLUDE%';
...
SELECT *
FROM MEMBER 
WHERE (MEMBER.EMAIL LIKE '%gmail.com%' AND LENGTH(MEMBER.EMAIL) >= 12) OR (MEMBER.EMAIL LIKE '%naver.com%' AND LENGTH(MEMBER.EMAIL) >= 10);
...
-- 결제 금액(ORDER_PRICE)이 100,000 이하면서 구매 개수가 10개 이상인 주문을 찾아주세요.

SELECT *
FROM PAYMENT_HISTORY 
WHERE (ORDER_PRICE >= 1000000 AND ORDER_COUNT <= 10) OR (ORDER_PRICE <= 100000 AND ORDER_COUNT >= 10);
...
SELECT *
FROM PAYMENT_HISTORY 
WHERE ((ORDER_PRICE >= 1000000 AND ORDER_COUNT <= 10) OR (ORDER_PRICE <= 100000 AND ORDER_COUNT >= 10)) AND ADDRESS  IN ('서울', '서울특별시', '서울시');

-- 미니 문제 #1
-- 아래 쿼리에서 컬럼명이 COUNT(1) 으로 나오게 되는데
-- COUNT(1) 이 아닌 TABLE_ROW_COUNT 로 나오게 해주세요.

SELECT COUNT(TABLE_ROW_COUNT) FROM MEMBER;
...
SELECT *
FROM PRODUCT
WHERE STOCK >= 10 ORDER BY STOCK DESC;
...
FROM   
    MEMBER, PAYMENT_HISTORY
WHERE
    MEMBER.ID = PAYMENT_HISTORY.MEMBER_ID AND PAYMENT_HISTORY.ORDER_PRICE >= 1000000 ORDER BY PAYMENT_HISTORY.ORDER_PRICE DESC;
